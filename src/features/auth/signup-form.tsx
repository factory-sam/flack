...
export function SignupForm() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      setMessage(
        "Check your email to finish setup. After confirmation, Flack creates your organization, #general, #random, and makes you admin."
      );
    } catch (error) {
        ...
        <form onSubmit={submit} className="flex flex-1 flex-col justify-center px-6 py-8">
          <div className="mb-6">
            ...
            <p className="mt-2 max-w-sm text-xs leading-5 text-[var(--muted)]">
              You will be the first admin. Flack creates default public channels and then you can invite the team.
            </p>
          </div>
      ...
      <section className="hidden min-h-0 flex-col justify-between p-6 md:flex">
        ...
        <div className="max-w-xl border-y border-[var(--line)] py-4">
          ...
          <p className="mt-2 text-xs leading-5 text-[var(--muted)]">
            Create the org, confirm your email, then land in #general with the admin invite controls ready. No sample
            data, no tour, no ceremony.
          </p>
        </div>
      </section>
    </main>
